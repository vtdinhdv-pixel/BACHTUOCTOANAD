...
import { Paperclip, Plus, Send, MessageSquareText, AlertTriangle, UserRound, X } from "lucide-react";
...
export default function ChatInterface({ 
  ...
  const calculateStatsFromMessages = (msgs: Message[]) => {
    ...
    const topics = {
      "Phân số": ["phân số", "tử số", "mẫu số", "rút gọn", "quy đồng", "hỗn số"],
      "Số nguyên": ["số nguyên", "âm", "dương", "giá trị tuyệt đối", "tập hợp z", "đối nhau"],
      "Hình học": ["hình học", "tam giác", "hình vuông", "chu vi", "diện tích", "góc", "đường thẳng", "điểm", "đoạn thẳng"],
      "Tính chia hết": ["chia hết", "ước", "bội", "số nguyên tố", "hợp số", "ucln", "bcnn"]
    };

    const correctKeywords = ["tốt lắm", "chính xác", "đúng rồi", "giỏi quá", "rất tốt", "hoàn thành", "hiểu bài"];
    const weakKeywords = ["làm lại nhé", "thử lại", "chưa đúng", "nhầm rồi", "cần xem lại", "sai rồi", "chưa chính xác"];
    ...
    // 1. Prioritize weak topics with specific actions
    ...
    const exploreActions: Record<string, string> = {
      "Phân số": "Khám phá các phép tính nhân chia phân số",
      "Số nguyên": "Tìm hiểu về nhân chia số nguyên và quy tắc dấu",
      "Hình học": "Làm quen với các loại góc và số đo góc",
      "Tính chia hết": "Tìm hiểu về số nguyên tố và hợp số"
    };
    ...
    // 2. Add topics not yet mastered
    const allTopics = Object.keys(topics);
    allTopics.forEach(topic => {
      if (!newStats.strong.includes(topic) && !newStats.weak.includes(topic)) {
        if (exploreActions[topic]) recommendations.push(exploreActions[topic]);
      }
    });

    // 3. Fallback or advanced suggestions
  ...
  useEffect(() => {
    ...
    // Reset Gemini chat context for the new session
    gemini.resetChat();
    ...
    const setupChat = async () => {
      ...
      // 1. Ensure session document exists FIRST
      ...
      unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        if (fetchedMessages.length === 0) {
          ...
          setStats(calculateStatsFromMessages(fetchedMessages));
        }
      }, (error) => {
  ...
  const handleTeacherRequest = () => {
    ...
    // Add a local message to indicate request sent
    const messagesRef = collection(db, "chatSessions", sessionId, "messages");
    addDoc(messagesRef, {
      role: "user",
      senderUid: auth.currentUser.uid,
